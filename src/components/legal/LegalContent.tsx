...
// 이용약관 · 개인정보처리방침 본문. 껍데기가 없는 순수 콘텐츠라 두 곳에서 같이 쓴다.
//  - LegalModal (홈 푸터에서 여는 팝업) — 기존 동작 그대로
//  - /terms · /privacy 페이지 — 구글 OAuth 동의 화면 등 외부에 제출할 고정 주소가 필요해서 추가
// 문구가 갈라지지 않도록 본문은 반드시 여기 한 곳에서만 관리한다.
export default function LegalContent({ kind }: { kind: LegalKind }) {
    return kind === 'terms' ? <TermsBody /> : <PrivacyBody />;
}
