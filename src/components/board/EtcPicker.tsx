    ...
    return (
        <button
            ...
            className={`h-16 border-2 border-dashed rounded-2xl flex items-center justify-center transition-all gap-2 mt-2 ${isDragging
                ...
                : 'border-gray-200 text-gray-400 hover:border-amber-400 hover:text-amber-500 hover:bg-amber-50'
                }`}
        >
            {isDragging ? (
                <>
...
function DraggableEtcCard({ card, index, cardId }: { card: any; index: number; cardId?: string }) {
    ...
    if (isDragging) {
        return (
            <div
                ref={setNodeRef}
                className="w-full h-[72px] border-2 border-dashed border-amber-300 bg-amber-50/50 rounded-lg"
            />
        );
    }
