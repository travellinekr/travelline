...
export function AccommodationAddModal({ destinationCity, anchorCoordinates, anchorTitle, onClose, onCreate }: AccommodationAddModalProps) {
    ...
    useEffect(() => {
        if (accommodationName.trim().length < 2) return;
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center md:p-4 bg-black/50">
            <div className="bg-white md:rounded-2xl shadow-2xl w-full md:max-w-2xl h-full md:max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-2.5 border-b border-gray-100 shrink-0">
                    <h2 className="text-lg font-bold text-slate-800">숙소 직접 추가하기</h2>
                    <button
                        onClick={onClose}
                    ...
                    <div>
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-9 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500"
                                onKeyPress={(e) => e.key === 'Enter' && handleSearch()}
                            />
                            {isSearching && (
                                <Loader2 className="w-4 h-4 text-rose-500 absolute right-3 top-1/2 -translate-y-1/2 animate-spin" />
                            )}
                        </div>
                    </div>
